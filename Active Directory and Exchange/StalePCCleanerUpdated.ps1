...
#Set time information. Computers that have not been logged into for 90 days are eligible to be retired. Computers that have not been logged into after 120 days are eligible to be delted from AD.
#Change the value after AddDays to customize the timeframes
$retirement = (Get-Date).AddDays(-90)
$removal = (Get-Date).AddDays(-120)
...
Get-ADComputer -Property Name,lastLogonDate,DistinguishedName,Enabled -Filter {lastlogondate -lt $retirement} -SearchBase 'OU=Computers,OU=USS,DC=win,DC=ad,DC=jhu,DC=edu' | Select-Object Name,lastLogonDate,DistinguishedName,Enabled | Where-Object {$_.DistinguishedName -notlike "CN=*,OU=USS-Retired,OU=Computers,OU=USS,DC=win,DC=ad,DC=jhu,DC=edu" -And $_.DistinguishedName -notlike "CN=*,OU=DMC-Gaming Loft,OU=DMC-Patron,OU=USS-DMC,OU=Computers,OU=USS,DC=win,DC=ad,DC=jhu,DC=edu" -And $_.DistinguishedName -notlike "CN=*,OU=USS-Exempted,OU=Computers,OU=USS,DC=win,DC=ad,DC=jhu,DC=edu"} |
ForEach-Object {
...
#If a computer is not reachable but it has been logged into within 60 days, the CSV will be updated and the LAN Admin should investigate.
#If a computer has not been logged into for 120 days (the $removal date), AND the lastlogondate field is not null, it will be deleted from AD
#If a computer has not been logged into for 60 days and it is still enabled in AD, it will be disabled.
...
if (Test-Connection $_.name -Count 1 -ErrorAction SilentlyContinue) {
...
}Elseif ($_.LastLogonDate -gt $retirement) {
  ...
  Get-ADComputer $_.DistinguishedName | Remove-ADObject -Confirm:$false -Whatif
  ...
  Get-ADComputer $_.DistinguishedName | Disable-ADAccount -Whatif
...
Send-MailMessage -From 'USSITServices <ussitservices@live.johnshopkins.edu>' -To 'USSITServices <ussitservices@live.johnshopkins.edu>' -Subject 'Stale PC Results' -Body "Results of Stale PC script attached" -Attachments \\win.ad.jhu.edu\cloud\HSA$\ITServices\Scripts\Results\StalePCs-$CurrentDate.csv -Priority High -DeliveryNotificationOption OnSuccess, OnFailure -SmtpServer 'smtp.johnshopkins.edu' 